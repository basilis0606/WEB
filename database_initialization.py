...
def mariadb_executemany_inserts_query( table_name, data, cur ):
    try:
        insert_query = "INSERT INTO " + table_name + " VALUES (" + (len(data[0])-1)*"?," +"?)"
    ...
    with open( product_categories_json,'r', encoding="utf8" ) as product_categories :
        ...
        for (category_id, category) in enumerate(categories, start=1):
            ...
            for subcategory in category.get("subcategories"):
                subcategory_id += 1
                subcatList.append( ( subcategory_id,category_id,subcategory.get("name") ) )
                for product in products:
                    if ( product.get("subcategory") == subcategory.get("uuid") ):
                        product_id += 1
                        productList.append( (product_id,product.get("name"),subcategory_id,INDICATOR.DEFAULT,INDICATOR.DEFAULT) )
...
def initialize_users ( users_json ):
    with open( users_json, 'r' ) as users:
        ...
        for user in data:
            usersList.append( (user["id"], user["username"], user["password"], user["email"], de,de,de,de ) )
        return usersList 
    ...
    with open( sales_json, 'r', encoding="utf8" ) as sales:
        ...
        for sale in data:
            ...
            for i in range(0,sale["dislikes"]):
                dislikesList.append( (INDICATOR.NULL,sale["id"],random.randrange(1,users_count)) )
...
def initialize_stores_from_overpass_turbo_query ( stores_json ):
    ...
    with open( stores_json, "r", encoding="utf8" ) as stores_from_overpass_query, open( "database_stores.json", "w" , encoding="utf8") as database_stores:
        ...
        for index,store in enumerate(data, start=1):
            store.pop("type")
            store["id"] = index
            store["tags"] = store["tags"].get("name")
            regional_markets.append((index, INDICATOR.DEFAULT, store["tags"]))
        json.dump(data, database_stores, ensure_ascii=False, indent=4)
...
def main():
    ...
    for arg in sys.argv:
        if ( arg == "categories" or arg == "subcategories" or arg == "products" ): cat_subcat_products = True
        if ( arg == "sales" or arg == "likes" or arg == "dislikes" ): sales = True 
        if ( arg == "users"): users = True 
        if ( arg == "stores" ): stores = True 

    if ( cat_subcat_products ):
        products_categories = initialize_cat_subcat_products( product_categories_json )
        mariadb_executemany_inserts_query( "categories", products_categories[0], cur)
        mariadb_executemany_inserts_query( "subcategories", products_categories[1], cur)
        mariadb_executemany_inserts_query( "products", products_categories[2], cur)
    if ( stores ):
        regional_stores = initialize_stores_from_overpass_turbo_query( regional_markets_json )
