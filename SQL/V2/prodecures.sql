CREATE PROCEDURE clear_monthly_user_data()
BEGIN
    UPDATE users SET monthly_score=0 , monthly_tokens=0;
END$
...
CREATE OR REPLACE PROCEDURE export_storesJson()
	BEGIN
		SELECT
			CONCAT(
				"{ 'type':'FeatureCollection','features':[",
				GROUP_CONCAT(
					CONCAT("{'type':'Feature','properties':{"),
					CONCAT("'id':",stores.id,",'name':",stores.name),
					CONCAT("},'geometry':{'coordinates':[",stores.lat,",",stores.lon,"],"),
					CONCAT("'type':'Point'}}")
				)
			,"]""}")
			AS json FROM stores
			INTO OUTFILE '~/Downloads/stores.json';
	END$
